...
```{r}
US_ports_data |>
  filter(port_name == "New York, NY & NJ" | port_name == "Baltimore, MD" | port_name == "Virginia, VA, Port of") |>
  filter(reporting_year == 2019) |>
  filter(units == "Container TEUs") |>
  filter(trade_type == "IMPORTS"| trade_type == "EXPORTS") |>
  select(port_name, reporting_year, trade_type, units, volume)

```

## Analyzing the Port of New York/New Jersey

The port of New York/New Jersey is the busiest of all ports located along the Atlantic Coast. With multiple airports, container terminals and warehousing, and an extensive rail network, the Port of NY/NJ i
