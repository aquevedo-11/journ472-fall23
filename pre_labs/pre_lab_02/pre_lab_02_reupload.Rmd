...
**Answer** A new variable simplifies existing data from a large dataset, making it easier to perform an analysis with only the information needed. 
...
**Answer** 41001. The OR operator gives us results that complete both filters (in this case, courses in the theater department or courses with greater than 15 seats).
...
**Answer:** The expenditure date column has undergone a formatting change (year, month, date vs. month, date, year) and is now being read by R as a date rather than a character set.
...
**Answer** The dates have been grouped by both month a year to avoid grouping a month (June, for example) across multiple years (2021 and 2022 in June's case).
...
**Answer** The code is creating two columns: total votes per county by adding together all ballots cast for candidates (Cox, Moore, Lasha, Wallace and write-ins) and percentage (as a decimal) of votes for Wes Moore by dividing the number of votes cast for Moore by the number of total votes found in the first newly created column.
...
**Answer** The data is now displayed in descending order from the county with the highest percentage of votes cast for Wes Moore (PG County) to lowest (Garrett County).
...
**Answer** Interesting, yes...maybe? The results show Moore received less than 50 percent of the vote in most counties (15 of 24). Maryland is known to be a solidly blue state, with the previous governer being an exception to the rule (though he was a moderate Republican). 
...
**Answer** str_to_lower() - converts text to all lower case; str_to_title() - converts text to title case (first letter of each word is capitalized)
...
**Answer** A new variable is being created. In the new variable sheet, the code is creating a separate column titled "state" and detecting all data in the address column that include words like "Maryland, California, Washington, etc" and filling the new "state" with said state abbreviations. NA is used for any addresses that do not contain the nine states coded.
...
**Answer** It makes sense that the campaign had expenses within the state of MD and some of its surrounding states; however, expenses in states like FL and MA seem too far out of reach?
...
1.  Which departments had the most courses offered with fewer than 300 seats available?

2.  What percentage of classes in any given department are lectures (say 100+ seats)?

3.  Which state had the largest indiviudal expense? Business/group/organization?
