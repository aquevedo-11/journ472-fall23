...
**Answer** There are multiple rows with grantee names that are essentially the same but have minor differences (i.e. Washington Adventist Total and Washington Adventist University).
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer** Lower cases entries changed to upper case ("Charles County Arts Alliance Inc" -> "CHARLES COUNTY ARTS ALLIANCE") & removal of special characters ("FAIR HILL FARM, INC." -> "FAIR HILL FARM INC").
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** Changed entries from uppercase to lowercase ("WASHINGTON ADVENTIST UNIVERSITY" -> "Washington Adventist University"); removed special characters ("WASHINGTON CO COMM. ACTION COUNCIL INC" -> "WASHINGTON CO COMM ACTION COUNCIL INC"); removed words from entry names ("WASHINGTON COUNTY COMMUNITY ACTION COUNCIL INC" -> "WASHINGTON COUNTY COMMUNITY ACTION COUNCIL")
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** ...I might be missing something but they look exactly the same to me?
