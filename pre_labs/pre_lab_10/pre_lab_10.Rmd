...
**Answer** Summary of shapefile including file location, number of rows and columns as well as column names. Most data falls under character (zipcode, county name associated with zip) or number type (shape length and area). There's also a multipolygon data type which contains info re: latitudes and longitudes of each zip which will be used to create a map.
...
**Answer** The code creates a dataframe based off the md_zips we previewed in the previous question. 526 rows, 8 columns containing info related to zipcodes (zipcode, name of county zip exists within, shape area and length, latitude and longitude values).
...
**Answer** Output is a map of the state of Maryland divided by zip code using the geometry values for allMD zipcodes in the md_zips dataset.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Tigris package has been loaded for use.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Similar to the shapefile in our first task, the below is a preview of all counties in the U.S. (3234 total). There are 18 columns, most are character values, a couple are numeric values and one contains multipolygon values.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** A new dataframe (md_counties) created filtering out any counties not in Maryland (code 24) from the original counties df.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** A map of Maryland divided by county rather than zip as in the previous map. The bay area has been excluded.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** The map of Maryland now includes both county and zip (different layers). Looks intense.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** A new df has been created using data from foreclosure_zip containing two columns: zip and count of foreclosures in March 2023. There is one more zip code than expected present in the df (belongs to DE).
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Df zip_codes_with_foreclosures and md_zips have been joined by zip codes. The DE zip is now gone. 
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** A map of Maryland uses a color scale to illustrate the number of foreclosures per zipcode (higher foreclosure # = lighter blue, lower foreclosure # - darker blue).
...
**Answer** Several libraries have been loaded for use including tidycensus.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Code generates df containing population per county in MD.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Same df aas abov, now with geometry column containing coordinates of each county.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** DF containing counts of foreclosure notices per county in the month of March (2023).
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Updated df containing county names (minus ", Maryland"), population, and geometry for mapping.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Df md_pop_with_foreclosures and md_county_population combined into one df containing county name, population, forclosure counts, and geometry for mapping.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**DF now with rate of foreclosures per 1000 people across all counties.
...
**Task** Run the following code. Describe the output in the space below. 
**Answer**  A heat map of Maryland illustrating the rate of foreclosures per county (higher foreclosure rate = lighter blue, lower foreclosure rate - darker blue).
...
**Task** Run the following code. Describe the output in the space below. 
**Answer** Same map, now with color scheme that makes it easier to differentiate rates as opposed to all-blue scheme.
...
**Answer**Same map, now with rate parameter adjusted so there is one fewer color in the heat map. Charles County remains the standout county; counties around the Chesapeake seem to have higher foreclosure rates than inland MD.
