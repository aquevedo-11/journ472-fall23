...
**Answer**The message alerts us to parsing issues and recommends we run a problem function to review the cause of the issue.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** "Vendor Name" and "Date" are character data types. "Amount" is a numberical data type.
...
**Answer**549 zip codes to not have five characters. These could be zip codes that begin with 0 or are international zips (Canadian zips are 6 digits long, 3 characters followed by a space and 3 more characters.)
...
**Answer** There are null results, categories other than "grant" or "loan" exist.
...
**Answer** A grant of $60 was awarded to the Land Reservation Trust, Inc. by the Department of Natural Resources/Rural Legacy Program in 2009.
...
**Answer** Headers now have underscores between words in the column names rather than spaces.
...
**Answer** The first line tells R which database to use. The second and third line change the column named grantor to source. It then mutates the source, grantee and description column texts to all uppercase to create uniformity in the data.
...
**Answer** There are 50 possible duplicate results.
...
**Answer** The new dataframe has 17,740 rows now as opposed to 17,765 rows in the original database.
...
**Answer** Zip5 codes now only contain 5 digit zipcodes as opposed to the 4- and 9-digit codes in the original zip_code column.
...
**Answer** The 21202 zip code belongs to Baltimore. It does make sense. As the state's largest city and top tourist destination, the Maryland Tourism Board priorities funding to Baltimore's downtown area. It has received over half a million dollars since 2009.

Filter, group by, summarize
filter - maryland tourism board
which zip code - zip5  - group by
