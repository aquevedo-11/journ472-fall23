...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** <chr> character; <dlb> double
...
**Answer** The code analyizes our data base (UMD classes) and groups courses by the term in which they were available to students. A count of courses available per per is also provided.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** The data has gone from being arranged by chronological order to arranged by number of courses provided (descending).
...
**Answer** Output now contains character and dbl data (previously dbl and int). The changed coding summarizes data by grouping data by course department and course term.
...
**Answer** Output no longer contains chr - data is now summarizes the number of classes offered and total seats available in each term.
...
**Answer** Data has been grouped by term and sorted by case and seat count in descending order. The average # and mean number of seats per class have also been calculated. All data is numeric, no text categories exist.
...
**Answer** Codeblock provides a deeper dive into the course makeup of each department - course count, seats available, mean/median, max/min seats.
...
1.  How many courses taught in any given term had more than 300 seats available?

2.  Which departments had the most courses offered with fewer than 300 seats available?

3.  Which departments had the largest decrease in courses offered from Spring 2020 to Spring 2021 (pre-pandemic semester to first full pandemic semester)
