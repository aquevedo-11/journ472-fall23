...
**A1** Most surprised by the smaller sizes of some STEM departments (biology, vet med sci, etc) - average is less than 10 seats per. 
...
**A3** Keith Yearwood, Geographical Sciences (excluding na responses)
...
**A4** Excluding all na results in our inquiry and would make it easier to find the answer; however, determining why the field is na (which instructor ultimately ended up teaching the course, if at all?) could change the number of classes taught by each instructor.
...
**A5Q** Question being answered: Which departments had the most courses offered with fewer than 300 seats available?
**A5A** The code below would be the first few steps taken to answer my question (it's wrong, I know). Additional code needed would tell R to exclude any classes with 301 seats or more from the class count.

```{r}
umd_courses |>
  group_by(department, seats) |>
  summarize(
    count_classes = n (),
